...
function Register() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      } else navigate('/register-workspace', { replace: true })
    ...
    <main className="min-h-screen bg-[var(--os-bg)] px-4 py-6 sm:px-6 lg:px-8">
      <div className="mx-auto grid min-h-[calc(100vh-3rem)] w-full max-w-6xl overflow-hidden rounded-[28px] border border-[var(--os-border)] bg-[var(--os-surface)] shadow-[var(--os-shadow-lg)] lg:grid-cols-[1.05fr_0.95fr]">
        <section className="relative hidden overflow-hidden border-r border-[var(--os-border)] bg-[var(--os-surface-raised)] p-10 lg:flex lg:flex-col lg:justify-between xl:p-14">
          <div className="absolute -left-24 -top-20 h-80 w-80 rounded-full bg-[var(--os-accent)] opacity-[0.08] blur-3xl" /><div className="absolute -bottom-28 -right-20 h-80 w-80 rounded-full bg-[var(--os-success)] opacity-[0.045] blur-3xl" />
        ...
        <section className="flex items-center justify-center p-6 sm:p-10 lg:p-12 xl:p-16"><div className="w-full max-w-md">
          ...
          <p className="text-sm font-medium text-[var(--os-accent)]">{isInvitedRegistration ? 'Workspace invitation' : 'Get started'}</p><h1 className="mt-2 text-3xl font-semibold tracking-tight text-[var(--os-text)] sm:text-4xl">{isInvitedRegistration ? 'Create your invited account' : 'Create your account'}</h1><p className="mt-3 text-sm leading-6 text-[var(--os-text-secondary)]">{isInvitedRegistration ? 'Complete registration to submit your invited workspace membership for approval.' : 'Set up your identity before you enter your Startup OS workspace.'}</p>
