...
import { getWorkspaceInvitations, revokeWorkspaceInvitation, subscribeToWorkspaceInvitations } from '../services/invitationService'
...
function Invitations() {
  ...
  async function load(workspaceId: string) {
    ...
    setError('')
    try {
      setItems(await getWorkspaceInvitations(workspaceId))
    } catch (e: unknown) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToWorkspaceInvitations(
      ...
      (listenError) => {
        if (!active) return
        setError(listenError.message || 'Unable to load invitations.')
        setLoading(false)
      },
    )
  ...
  async function revoke(item: WorkspaceInvitation) {
    ...
    setError('')
    try {
      await revokeWorkspaceInvitation(workspace.id, item.id)
    } catch (e: unknown) {
  ...
  async function copy(item: WorkspaceInvitation) {
    ...
    url.searchParams.set('inviteToken', item.token)
    if (item.email) url.searchParams.set('email', item.email)
    try {
