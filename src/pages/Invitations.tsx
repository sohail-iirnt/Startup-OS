...
function Invitations() {
  ...
  async function load(workspaceId: string, showLoading = true) {
    if (showLoading) setLoading(true)
    setError('')
    try {
      const next = await getWorkspaceInvitations(workspaceId)
      setItems(next)
    } catch (e: unknown) {
  ...
  const filtered = useMemo(() => items.filter((item) => { const status = statusOf(item); const q = search.trim().toLowerCase(); return (filter === 'all' || status === filter) && (!q || (item.email || 'open invitation').toLowerCase().includes(q) || label(item.role).toLowerCase().includes(q)) }), [items, filter, search])

  async function revoke(item: WorkspaceInvitation) { if (!workspace?.id || statusOf(item) !== 'pending') return; setBusy(item.id); setError(''); try { await revokeWorkspaceInvitation(workspace.id, item.id); setItems((current) => current.map((x) => x.id === item.id ? { ...x, status: 'revoked', revokedAt: new Date() } : x)) } catch (e: unknown) { setError(e instanceof Error ? e.message : 'Unable to revoke invitation.') } finally { setBusy('') } }
  async function copy(item: WorkspaceInvitation) { if (!workspace?.id) return; const url = new URL('/register', window.location.origin); url.searchParams.set('workspaceId', workspace.id); url.searchParams.set('inviteToken', item.token); if (item.email) url.searchParams.set('email', item.email); try { await navigator.clipboard.writeText(url.toString()); setCopied(item.id); window.setTimeout(() => setCopied(''), 1800) } catch { setError('Unable to copy invitation link.') } }
