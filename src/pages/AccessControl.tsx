...
function AccessControl() {
    ...
    const applyMembers = (items: WorkspaceMember[]) => {
      ...
      if (!initialised) {
        ...
        if (first) {
          ...
          setSelectedMemberId(first.userId); setGranted(nextGranted); setDenied(nextDenied); setSavedGranted(nextGranted); setSavedDenied(nextDenied)
        }
        initialised = true
      }
    }
    getWorkspaceMembers(workspace.id).then(applyMembers).catch((loadError: unknown) => { if (active) setError(loadError instanceof Error ? loadError.message : 'Unable to load workspace members.') }).finally(() => { if (active) setLoading(false) })
    const unsubscribe = subscribeToWorkspaceMembers(workspace.id, applyMembers, (listenError) => { if (active) setError(listenError.message) })
  ...
  const hasChanges = !samePermissions(granted, savedGranted) || !samePermissions(denied, savedDenied)

  function selectMember(userId: string) {
    const next = members.find((item) => item.userId === userId)
    const nextGranted = next?.grantedPermissions ?? []; const nextDenied = next?.deniedPermissions ?? []
    setSelectedMemberId(userId); setGranted(nextGranted); setDenied(nextDenied); setSavedGranted(nextGranted); setSavedDenied(nextDenied); setMessage(''); setError('')
  }
  function togglePermission(permission: WorkspacePermission) {
    if (!selectedMember || selectedMember.role === 'owner' || saving) return
    const state = getPermissionState({ ...selectedMember, grantedPermissions: granted, deniedPermissions: denied }, permission)
    setMessage(''); setError('')
    if (state === 'inherited') { setDenied((current) => current.includes(permission) ? current : [...current, permission]); setGranted((current) => current.filter((item) => item !== permission)) }
    else if (state === 'denied') setDenied((current) => current.filter((item) => item !== permission))
    else if (state === 'granted') setGranted((current) => current.filter((item) => item !== permission))
    else { setGranted((current) => current.includes(permission) ? current : [...current, permission]); setDenied((current) => current.filter((item) => item !== permission)) }
  }
  function resetToRoleDefaults() { if (!selectedMember || selectedMember.role === 'owner' || saving) return; setGranted([]); setDenied([]); setMessage('Custom overrides cleared. Save to restore this member to the role defaults.'); setError('') }
  function grantAllPermissions() { if (!selectedMember || selectedMember.role === 'owner' || saving) return; const rolePermissions = new Set(getRolePermissions(selectedMember.role)); setGranted(ALL_PERMISSIONS.filter((permission) => !rolePermissions.has(permission))); setDenied([]); setMessage('All available permissions selected. Save to apply.'); setError('') }
  function denyAllOptionalPermissions() { if (!selectedMember || selectedMember.role === 'owner' || saving) return; setGranted([]); setDenied([...getRolePermissions(selectedMember.role)]); setMessage('Role permissions selected for explicit denial. Save to apply.'); setError('') }
  async function savePermissions() {
    if (!workspace?.id || !selectedMember || selectedMember.role === 'owner' || saving || !hasChanges) return
    const memberName = selectedMember.displayName || selectedMember.email || 'this member'; const nextGranted = [...new Set(granted)]; const nextDenied = [...new Set(denied)].filter((permission) => !nextGranted.includes(permission))
    setSaving(true); setError(''); setMessage('Saving permission changes…')
    try { await updateMemberPermissions(workspace.id, selectedMember.userId, nextGranted, nextDenied); setMembers((current) => current.map((item) => item.userId === selectedMember.userId ? { ...item, grantedPermissions: nextGranted, deniedPermissions: nextDenied } : item)); setGranted(nextGranted); setDenied(nextDenied); setSavedGranted(nextGranted); setSavedDenied(nextDenied); setMessage(`Permissions saved successfully for ${memberName}.`) }
    catch (saveError: unknown) { setError(saveError instanceof Error ? saveError.message : 'Unable to save permissions. Please try again.'); setMessage('') }
    finally { setSaving(false) }
  }

  if (!canManage) return <div className="mx-auto w-full max-w-[1200px] p-4 sm:p-6 lg:p-8"><Card className="p-8 text-center"><LockKeyhole className="mx-auto text-[var(--os-warning)]" size={28} /><h1 className="mt-4 text-xl font-semibold text-[var(--os-text)]">Permission management restricted</h1><p className="mt-2 text-sm text-[var(--os-text-secondary)]">Only workspace owners and authorized administrators can customize member permissions.</p></Card></div>
  ...
  return <div className="mx-auto w-full max-w-[1400px] p-4 sm:p-6 lg:p-8">
    ...
    <Card className="mb-6 p-5 sm:p-6"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="flex items-center gap-3"><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-[var(--os-accent-soft)] text-[var(--os-accent)]"><UserCheck size={20} /></div><div><h2 className="text-base font-semibold text-[var(--os-text)]">Customize a member</h2><p className="mt-1 text-sm text-[var(--os-text-secondary)]">Choose an active member, then tick or revoke individual capabilities.</p></div></div><div className="relative w-full lg:max-w-md"><UserRound size={17} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[var(--os-text-muted)]" /><select value={selectedMemberId} onChange={(event) => selectMember(event.target.value)} disabled={loading || saving} className="os-focus-ring h-11 w-full appearance-none rounded-xl border border-[var(--os-border)] bg-[var(--os-surface-raised)] pl-10 pr-10 text-sm text-[var(--os-text)]">{members.map((item) => <option key={item.userId} value={item.userId}>{item.displayName || item.email || 'Unnamed member'} — {roleLabel(item.role)}</option>)}</select><ChevronDown size={16} className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-[var(--os-text-muted)]" /></div></div></Card>
    {error && <div role="alert" className="mb-5 rounded-xl border border-[rgba(255,100,124,0.25)] bg-[rgba(255,100,124,0.08)] px-4 py-3 text-sm text-[var(--os-danger)]">{error}</div>}{message && <div role="status" className="mb-5 rounded-xl border border-[rgba(74,222,128,0.25)] bg-[rgba(74,222,128,0.08)] px-4 py-3 text-sm text-[var(--os-success)]">{message}</div>}
    {selectedMember && <><Card className="mb-6 p-5 sm:p-6"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="flex items-center gap-3"><ShieldCheck size={21} className="text-[var(--os-accent)]" /><div><h2 className="text-lg font-semibold text-[var(--os-text)]">{selectedMember.displayName || selectedMember.email || 'Unnamed member'}</h2><p className="mt-1 text-sm text-[var(--os-text-secondary)]">{selectedMember.email || 'No email'} · {roleLabel(selectedMember.role)}</p></div></div><div className="flex flex-wrap gap-2 text-xs"><span className="rounded-full border border-[var(--os-border)] bg-[var(--os-surface-raised)] px-3 py-1.5 text-[var(--os-text-secondary)]">{selectedPermissions.size} / {permissionGroups.flatMap((group) => group.permissions).length} effective</span><span className="rounded-full border border-[var(--os-border)] bg-[var(--os-surface-raised)] px-3 py-1.5 text-[var(--os-text-secondary)]">{granted.length} custom grants</span><span className="rounded-full border border-[var(--os-border)] bg-[var(--os-surface-raised)] px-3 py-1.5 text-[var(--os-text-secondary)]">{denied.length} custom denials</span></div></div></Card>
    ...
    {selectedMember.role !== 'owner' && <Card className="mb-5 p-4"><div className="flex flex-wrap gap-2"><button type="button" onClick={grantAllPermissions} disabled={saving} className="os-focus-ring rounded-xl border border-[var(--os-border)] bg-[var(--os-surface-raised)] px-3 py-2 text-xs font-semibold text-[var(--os-text)] disabled:opacity-50">Grant all</button><button type="button" onClick={denyAllOptionalPermissions} disabled={saving} className="os-focus-ring rounded-xl border border-[var(--os-border)] bg-[var(--os-surface-raised)] px-3 py-2 text-xs font-semibold text-[var(--os-text)] disabled:opacity-50">Remove role access</button><button type="button" onClick={resetToRoleDefaults} disabled={saving} className="os-focus-ring rounded-xl border border-[var(--os-border)] bg-[var(--os-surface-raised)] px-3 py-2 text-xs font-semibold text-[var(--os-text)] disabled:opacity-50">Reset to role defaults</button><span className="self-center text-xs text-[var(--os-text-muted)]">Use individual permissions below for precise custom access.</span></div></Card>}
    ...
    <div className="sticky bottom-4 z-10 mt-6 flex justify-end"><button type="button" onClick={() => void savePermissions()} disabled={saving || selectedMember.role === 'owner' || !hasChanges} className="os-focus-ring inline-flex h-11 min-w-[190px] items-center justify-center rounded-xl bg-[var(--os-accent)] px-5 text-sm font-semibold text-white shadow-lg transition-opacity disabled:cursor-not-allowed disabled:opacity-60">{saving ? 'Saving permissions…' : hasChanges ? 'Save permission changes' : 'Permissions saved'}</button></div></>}
  </div>
}
