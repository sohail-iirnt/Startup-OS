...
function AccessControl() {
  ...
  const selectedPermissions = useMemo(
    () => new Set(getRolePermissions(selectedRole)),
    [selectedRole],
  )
  ...
  return (
    <div className="mx-auto w-full max-w-[1400px] p-4 sm:p-6 lg:p-8">
      <section className="mb-8">
        ...
        <p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--os-text-secondary)] sm:text-base">
          Review the current Startup OS access model before assigning responsibilities across your workspace.
        </p>
      </section>
      ...
      <div className="grid gap-6 lg:grid-cols-[280px_minmax(0,1fr)]">
        <Card className="h-fit p-5">
          <SectionHeader
            title="Roles"
            description="Current workspace access profiles."
          />
            ...
            {roles.map((role) => {
              ...
              return (
                <button
                  ...
                  className={`os-focus-ring flex w-full items-center justify-between rounded-xl border px-4 py-3 text-left transition-colors ${
                    active
                      ? 'border-[var(--os-accent-border)] bg-[var(--os-accent-soft)]'
                      : 'border-[var(--os-border)] hover:bg-[var(--os-surface-hover)]'
                  }`}
                >
                  <span>
                    <span className="block text-sm font-semibold text-[var(--os-text)]">{roleLabel(role)}</span>
                    <span className="mt-0.5 block text-xs text-[var(--os-text-muted)]">{count} permissions</span>
                  </span>
                  {active && <Check size={17} className="text-[var(--os-accent)]" />}
                </button>
              )
            })}
          </div>

          <div className="mt-5 rounded-xl border border-[var(--os-border)] bg-[var(--os-surface-raised)] p-4">
            <div className="flex items-start gap-3">
              <LockKeyhole size={17} className="mt-0.5 shrink-0 text-[var(--os-text-muted)]" />
              <p className="text-xs leading-5 text-[var(--os-text-secondary)]">
                Access is enforced through workspace membership, navigation visibility, and protected application routes.
              </p>
            </div>
          </div>
        </Card>
        ...
        <div className="space-y-6">
          <Card className="p-5 sm:p-6">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center gap-3">
                <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-[var(--os-accent-soft)] text-[var(--os-accent)]">
                  <ShieldCheck size={20} />
                </div>
                <div>
                  <h2 className="text-lg font-semibold text-[var(--os-text)]">{roleLabel(selectedRole)} access profile</h2>
                  <p className="mt-1 text-sm text-[var(--os-text-secondary)]">
                    {permissionCount} of {totalPermissions} currently available permissions
                  </p>
                </div>
              </div>
              <span className="inline-flex w-fit rounded-full border border-[var(--os-border)] bg-[var(--os-surface-raised)] px-3 py-1.5 text-xs font-medium capitalize text-[var(--os-text-secondary)]">
                {selectedRole}
              </span>
            </div>
          </Card>
          ...
          {permissionGroups.map((group) => (
            <Card key={group.label} className="overflow-hidden">
              <div className="border-b border-[var(--os-border)] px-5 py-4 sm:px-6">
                <h3 className="text-sm font-semibold text-[var(--os-text)]">{group.label}</h3>
              </div>
              <div className="divide-y divide-[var(--os-border)]">
                {group.permissions.map((permission) => {
                  const enabled = selectedPermissions.has(permission.key)
                  return (
                    <div key={permission.key} className="flex items-center justify-between gap-4 px-5 py-4 sm:px-6">
                      <div className="min-w-0">
                        <p className="text-sm font-medium text-[var(--os-text)]">{permission.label}</p>
                        <p className="mt-1 text-xs leading-5 text-[var(--os-text-muted)]">{permission.description}</p>
                        <p className="mt-1 font-mono text-[10px] text-[var(--os-text-muted)]">{permission.key}</p>
                      </div>
                      <div
                        aria-label={enabled ? 'Permission enabled' : 'Permission disabled'}
                        className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border ${
                          enabled
                            ? 'border-[var(--os-success)]/30 bg-[var(--os-success)]/10 text-[var(--os-success)]'
                            : 'border-[var(--os-border)] bg-[var(--os-surface-raised)] text-[var(--os-text-muted)]'
                        }`}
                      >
                        {enabled ? <Check size={15} /> : <span className="text-xs">—</span>}
                      </div>
                    </div>
                  )
                })}
          ...
          <div className="rounded-xl border border-dashed border-[var(--os-border)] px-5 py-4 text-xs leading-5 text-[var(--os-text-muted)]">
            This screen currently reflects the platform's built-in role matrix. Workspace-specific permission editing will be added only after the underlying Firestore role configuration and security rules are ready, so the UI never promises access that the backend cannot enforce.
          </div>
        </div>
      </div>
    </div>
