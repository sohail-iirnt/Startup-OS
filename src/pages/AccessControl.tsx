...
import { getWorkspaceMembers, updateMemberPermissions } from '../services/workspaceService'
...
function AccessControl() {
  ...
  useEffect(() => {
    ...
    getWorkspaceMembers(workspace.id).then((items) => {
      if (!active) return
      setMembers(items)
      setSelectedMemberId((current) => current || items.find((item) => item.userId !== currentMember?.userId)?.userId || items[0]?.userId || '')
  ...
  useEffect(() => {
    if (!selectedMember) { setGranted([]); setDenied([]); return }
  ...
  return (
    <div className="mx-auto w-full max-w-[1400px] p-4 sm:p-6 lg:p-8">
      ...
      {selectedMember && <>
        <Card className="mb-6 p-5 sm:p-6"><div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between"><div><div className="flex items-center gap-3"><ShieldCheck size={21} className="text-[var(--os-accent)]" /><div><h2 className="text-lg font-semibold text-[var(--os-text)]">{selectedMember.displayName || selectedMember.email || 'Unnamed member'}</h2><p className="mt-1 text-sm text-[var(--os-text-secondary)]">{selectedMember.email || 'No email'} · {roleLabel(selectedMember.role)}</p></div></div></div><div className="flex flex-wrap gap-2 text-xs"><span className="rounded-full border border-[var(--os-border)] bg-[var(--os-surface-raised)] px-3 py-1.5 text-[var(--os-text-secondary)]">{selectedPermissions.size} / {totalPermissions} effective</span><span className="rounded-full border border-[var(--os-border)] bg-[var(--os-surface-raised)] px-3 py-1.5 text-[var(--os-text-secondary)]">{granted.length} custom grants</span><span className="rounded-full border border-[var(--os-border)] bg-[var(--os-surface-raised)] px-3 py-1.5 text-[var(--os-text-secondary)]">{denied.length} custom denials</span></div></div></Card>
