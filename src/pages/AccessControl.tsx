...
function samePermissions(left: WorkspacePermission[], right: WorkspacePermission[]) {
...
function PermissionEditor({ member, workspaceId }: PermissionEditorProps) {
  ...
  const [denied, setDenied] = useState<WorkspacePermission[]>(() => [...(member.deniedPermissions ?? [])])
  const [savedGranted, setSavedGranted] = useState<WorkspacePermission[]>(() => [...(member.grantedPermissions ?? [])])
  const [savedDenied, setSavedDenied] = useState<WorkspacePermission[]>(() => [...(member.deniedPermissions ?? [])])
  const [saving, setSaving] = useState(false)
  const [message, setMessage] = useState('')
  const [error, setError] = useState('')

  const selectedPermissions = useMemo(() => {
    const result = new Set<WorkspacePermission>(getRolePermissions(member.role))
    granted.forEach(permission => result.add(permission))
    denied.forEach(permission => result.delete(permission))
    return result
  }, [member.role, granted, denied])
  ...
  function togglePermission(permission: WorkspacePermission) {
    if (member.role === 'owner' || saving) return
    setMessage('')
    setError('')
    const roleDefault = getRolePermissions(member.role).includes(permission)
    if (denied.includes(permission)) {
      setDenied(current => current.filter(item => item !== permission))
    } else if (granted.includes(permission)) {
      setGranted(current => current.filter(item => item !== permission))
    } else if (roleDefault) {
      setDenied(current => [...current, permission])
    } else {
      setGranted(current => [...current, permission])
    }
  }

  async function savePermissions() {
    if (member.role === 'owner' || !dirty || saving) return
    setSaving(true)
    setMessage('')
    setError('')
    try {
      await updateMemberPermissions(workspaceId, member.userId, granted, denied)
      setSavedGranted([...granted])
      setSavedDenied([...denied])
  ...
  function resetPermissions() {
    setGranted([...savedGranted])
    setDenied([...savedDenied])
    setMessage('')
    setError('')
  }
    ...
    <Card className="mb-5 p-5">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-3">
          <div className="flex h-11 w-11 items-center justify-center rounded-full bg-[var(--os-accent-soft)] text-[var(--os-accent)]"><UserRound size={19} /></div>
          <div><h2 className="text-base font-semibold text-[var(--os-text)]">{member.displayName || 'Unnamed member'}</h2><p className="text-xs text-[var(--os-text-muted)]">{member.email} · {roleLabel(member.role)}</p></div>
        </div>
        <div className="flex items-center gap-2">
          <span className={`rounded-full px-2.5 py-1 text-[10px] font-semibold ${member.role === 'owner' ? 'bg-[var(--os-accent-soft)] text-[var(--os-accent)]' : 'bg-[var(--os-success-soft)] text-[var(--os-success)]'}`}>{member.role === 'owner' ? 'Owner — locked' : dirty ? 'Unsaved changes' : 'Saved'}</span>
          <button type="button" disabled={!dirty || saving || member.role === 'owner'} onClick={resetPermissions} className="rounded-xl border border-[var(--os-border)] px-3 py-2 text-xs font-semibold text-[var(--os-text-secondary)] disabled:opacity-50">Reset</button>
          <button type="button" disabled={!dirty || saving || member.role === 'owner'} onClick={() => void savePermissions()} className="rounded-xl bg-[var(--os-accent)] px-4 py-2 text-xs font-semibold text-white disabled:opacity-50">{saving ? 'Saving…' : 'Save permissions'}</button>
        </div>
      </div>
      {message && <p className="mt-4 rounded-xl bg-[var(--os-success-soft)] p-3 text-sm text-[var(--os-success)]">{message}</p>}
      {error && <p className="mt-4 rounded-xl bg-[var(--os-danger-soft)] p-3 text-sm text-[var(--os-danger)]">{error}</p>}
    </Card>
    <div className="space-y-6">
      {permissionGroups.map(group => <Card key={group.label} className="overflow-hidden">
        <div className="border-b border-[var(--os-border)] px-5 py-4 sm:px-6"><h3 className="text-sm font-semibold text-[var(--os-text)]">{group.label}</h3></div>
        <div className="divide-y divide-[var(--os-border)]">
          {group.permissions.map(permission => {
            const state = getPermissionState({ ...member, grantedPermissions: granted, deniedPermissions: denied }, permission.key)
            const effective = selectedPermissions.has(permission.key)
            const disabled = member.role === 'owner' || saving
            return <button key={permission.key} type="button" disabled={disabled} onClick={() => togglePermission(permission.key)} className="flex w-full items-center justify-between gap-4 px-5 py-4 text-left transition-colors hover:bg-[var(--os-surface-hover)] disabled:cursor-not-allowed disabled:opacity-70 sm:px-6">
              <div className="min-w-0"><p className="text-sm font-medium text-[var(--os-text)]">{permission.label}</p><p className="mt-1 text-xs leading-5 text-[var(--os-text-muted)]">{permission.description}</p><p className="mt-1 font-mono text-[10px] text-[var(--os-text-muted)]">{permission.key}</p></div>
              <div className="flex shrink-0 items-center gap-2"><span className={`hidden rounded-full px-2.5 py-1 text-[10px] font-semibold sm:inline-flex ${state === 'inherited' ? 'bg-[var(--os-success-soft)] text-[var(--os-success)]' : state === 'granted' ? 'bg-[var(--os-accent-soft)] text-[var(--os-accent)]' : state === 'denied' ? 'bg-[var(--os-danger-soft)] text-[var(--os-danger)]' : 'bg-[var(--os-surface-hover)] text-[var(--os-text-muted)]'}`}>{state}</span><span className={`flex h-9 w-9 items-center justify-center rounded-xl border ${effective ? 'border-[var(--os-success)]/30 bg-[var(--os-success-soft)] text-[var(--os-success)]' : 'border-[var(--os-border)] bg-[var(--os-surface-raised)] text-[var(--os-text-muted)]'}`}>{effective ? <Check size={16} /> : <span className="text-xs">—</span>}</span></div>
            </button>
          })}
        </div>
      </Card>)}
...
function AccessControl() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToWorkspaceMembers(workspace.id, applyMembers, listenerError => {
      if (!active) return
  ...
  return <div className="mx-auto w-full max-w-[1400px] p-4 sm:p-6 lg:p-8">
    <div className="mb-6"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--os-accent)]">Security & access</p><h1 className="mt-1 text-3xl font-semibold tracking-tight text-[var(--os-text)]">Roles & Permissions</h1><p className="mt-2 max-w-3xl text-sm text-[var(--os-text-secondary)]">Select an active member and customize exactly what they can see or manage. Role defaults are inherited; custom grants and denials override them.</p></div>
    <Card className="mb-5 p-5"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[var(--os-accent-soft)] text-[var(--os-accent)]"><ShieldCheck size={19} /></div><div><p className="text-sm font-semibold text-[var(--os-text)]">Member access editor</p><p className="text-xs text-[var(--os-text-muted)]">{loading ? 'Loading members…' : `${members.length} active workspace members`}</p></div></div><select value={selectedMemberId} onChange={event => setSelectedMemberId(event.target.value)} disabled={loading} className="os-focus-ring h-11 min-w-[280px] rounded-xl border border-[var(--os-border)] bg-[var(--os-surface-raised)] px-3 text-sm text-[var(--os-text)]"><option value="">Select member</option>{members.map(item => <option key={item.id} value={item.id}>{item.displayName || item.email || item.userId} · {roleLabel(item.role)}</option>)}</select></div></Card>
    {error && <Card className="mb-5 p-4 text-sm text-[var(--os-danger)]">{error}</Card>}
    {selectedMember ? <PermissionEditor key={selectedMember.id} member={selectedMember} workspaceId={workspace!.id} /> : !loading && <Card className="p-12 text-center"><UserCheck className="mx-auto text-[var(--os-text-muted)]" size={34} /><h2 className="mt-4 text-lg font-semibold text-[var(--os-text)]">Choose a member</h2><p className="mt-1 text-sm text-[var(--os-text-secondary)]">Select a workspace member above to customize their access.</p></Card>}
  </div>
}
