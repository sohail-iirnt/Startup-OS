...
function initials(member: WorkspaceMember) {
  ...
  return source.split(/\s+/).filter(Boolean).slice(0, 2).map((part) => part.charAt(0).toUpperCase()).join('') || 'M'
}
...
function isOverdue(task: Task) {
  return !!task.dueDate && task.status !== 'completed' && task.status !== 'cancelled' && task.dueDate.getTime() < Date.now()
}
    ...
    <div className="mx-auto w-full max-w-[1400px] p-4 sm:p-6 lg:p-8">
      ...
      <Card className="mb-5 p-4 sm:p-5">
        <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-center gap-2"><Filter size={17} className="text-[var(--os-accent)]" /><div><h2 className="text-sm font-semibold text-[var(--os-text)]">Member Directory</h2><p className="mt-0.5 text-xs text-[var(--os-text-secondary)]">Showing {filteredMembers.length} of {members.length} members</p></div></div>
          {hasFilters && <button type="button" onClick={clearFilters} className="inline-flex items-center gap-1.5 self-start text-xs font-semibold text-[var(--os-accent)] hover:text-[var(--os-accent-hover)]">Clear filters <X size={14} /></button>}
        </div>
        <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_180px_180px]">
          <label className="relative block"><Search size={16} className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-[var(--os-text-muted)]" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search name, email, role or designation..." className="os-focus-ring h-11 w-full rounded-xl border border-[var(--os-border)] bg-[var(--os-surface-raised)] pl-10 pr-3 text-sm text-[var(--os-text)] placeholder:text-[var(--os-text-muted)]" /></label>
          <select value={roleFilter} onChange={(event) => setRoleFilter(event.target.value)} className="os-focus-ring h-11 rounded-xl border border-[var(--os-border)] bg-[var(--os-surface-raised)] px-3 text-sm text-[var(--os-text)]"><option value="all">All roles</option>{roles.map((role) => <option key={role} value={role}>{roleLabel(role)}</option>)}</select>
          <select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} className="os-focus-ring h-11 rounded-xl border border-[var(--os-border)] bg-[var(--os-surface-raised)] px-3 text-sm text-[var(--os-text)]"><option value="all">All statuses</option><option value="active">Active</option><option value="suspended">Suspended</option><option value="pending">Pending</option><option value="rejected">Rejected</option></select>
        </div>
      </Card>

      {loading || workspaceLoading ? <Card className="p-8 text-center text-sm text-[var(--os-text-secondary)]">Loading team members and workload...</Card> : filteredMembers.length === 0 ? <Card className="p-10 text-center"><div className="mx-auto flex h-12 w-12 items-center justify-center rounded-xl bg-[var(--os-accent-soft)] text-[var(--os-accent)]"><UserRound size={22} /></div><h2 className="mt-4 text-lg font-semibold text-[var(--os-text)]">{hasFilters ? 'No members match your filters' : 'No active members yet'}</h2><p className="mx-auto mt-2 max-w-lg text-sm leading-6 text-[var(--os-text-secondary)]">{hasFilters ? 'Try changing the search or filters to find another workspace member.' : 'Registered workspace members will appear here and can then be selected as assignees throughout Startup OS.'}</p>{hasFilters && <button type="button" onClick={clearFilters} className="mt-5 rounded-xl border border-[var(--os-border)] px-4 py-2 text-sm font-semibold text-[var(--os-text)] hover:bg-[var(--os-surface-hover)]">Clear filters</button>}</Card> : <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
