...
import { CheckCircle2, Clock3, Filter, Mail, Search, ShieldCheck, UserPlus, UserRound, Users, Workflow } from 'lucide-react'
import { Link } from 'react-router-dom'
import Card from '../components/ui/Card'
import InviteMemberModal from '../components/members/InviteMemberModal'
import { useAuth } from '../context/useAuth'
import { useWorkspace } from '../context/useWorkspace'
import { getWorkspaceMemberDetails, getWorkspaceMembers } from '../services/workspaceService'
  ...
  useEffect(() => {
    ...
    async function start() {
      try {
        ...
        unsubscribeTasks = await subscribeToTasks(workspaceId, next => {
          if (!active) return
          setTasks(isIntern ? next.filter(task => task.assigneeId === userId || task.createdBy === userId) : next)
          setLoading(false)
          setError('')
        }, listenError => { if (active) { setError(listenError.message); setLoading(false) } })
    ...
    void start()
    return () => { active = false; unsubscribeTasks?.() }
