...
import { CheckCircle2, Clock3, Mail, ShieldCheck, UserPlus, UserRound, Users, Workflow } from 'lucide-react'
...
function Team() {
  ...
  return (
    <div className="mx-auto w-full max-w-[1400px] p-4 sm:p-6 lg:p-8">
      ...
      {loading || workspaceLoading ? <Card className="p-8 text-center text-sm text-[var(--os-text-secondary)]">Loading team members and workload...</Card> : members.length === 0 ? <Card className="p-10 text-center"><div className="mx-auto flex h-12 w-12 items-center justify-center rounded-xl bg-[var(--os-accent-soft)] text-[var(--os-accent)]"><UserRound size={22} /></div><h2 className="mt-4 text-lg font-semibold text-[var(--os-text)]">No active members yet</h2><p className="mx-auto mt-2 max-w-lg text-sm leading-6 text-[var(--os-text-secondary)]">Registered workspace members will appear here and can then be selected as assignees throughout Startup OS.</p></Card> : <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {members.map((member) => {
