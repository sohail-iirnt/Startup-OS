    ...
    <div className="mx-auto w-full max-w-[1400px] p-4 sm:p-6 lg:p-8">
      ...
      <div className="mb-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Card className="p-5">
          <div className="flex items-center gap-3">
            <Users size={20} className="text-[var(--os-accent)]" />
            <div>
              <p className="text-xs text-[var(--os-text-muted)]">Active Members</p>
              <p className="mt-1 text-2xl font-semibold text-[var(--os-text)]">
                {members.length}
              </p>
            </div>
          </div>
        ...
        <Card className="p-5">
          <div className="flex items-center gap-3">
            <Workflow size={20} className="text-[var(--os-info)]" />
            <div>
        ...
        <Card className="p-5">
          <div className="flex items-center gap-3">
            <CheckCircle2 size={20} className="text-[var(--os-success)]" />
            <div>
              <p className="text-xs text-[var(--os-text-muted)]">Completed Tasks</p>
              <p className="mt-1 text-2xl font-semibold text-[var(--os-text)]">
        ...
        <Card className="p-5">
          <div className="flex items-center gap-3">
            <Clock3 size={20} className="text-[var(--os-warning)]" />
            <div>
              <p className="text-xs text-[var(--os-text-muted)]">Overdue Tasks</p>
              <p className="mt-1 text-2xl font-semibold text-[var(--os-text)]">
                {overdueTasks.length}
              </p>
            </div>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-[var(--os-text-secondary)]">
              {assignedTasks.length} assigned tasks · {unassignedTasks.length} unassigned tasks · {tasks.length} total tasks
            </p>
          </div>
          <div className="h-2 w-full max-w-xs overflow-hidden rounded-full bg-[var(--os-surface-hover)]">
            <div
              className="h-full rounded-full bg-[var(--os-accent)] transition-all"
              style={{
                width: `${tasks.length ? Math.min(100, (assignedTasks.length / tasks.length) * 100) : 0}%`,
              }}
            />
          </div>
        </div>
      ...
      <div className="mb-5">
        <Card className="p-5">
          <div className="flex items-center gap-2">
            <ShieldCheck size={18} className="text-[var(--os-success)]" />
            <div>
              <h2 className="text-sm font-semibold text-[var(--os-text)]">
                Workspace
              </h2>
              <p className="mt-0.5 text-xs text-[var(--os-text-secondary)]">
      ...
      {loading || workspaceLoading ? (
        <Card className="p-8 text-center text-sm text-[var(--os-text-secondary)]">
          Loading team members and workload...
        </Card>
      ) : members.length === 0 ? (
        <Card className="p-10 text-center">
          ...
          <p className="mx-auto mt-2 max-w-lg text-sm leading-6 text-[var(--os-text-secondary)]">
            Registered workspace members will appear here and can then be selected as assignees throughout Startup OS.
          </p>
        </Card>
      ) : (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {members.map((member) => {
            ...
            const memberActiveTasks = memberTaskList.filter(
              (task) =>
                task.status !== 'completed' &&
                task.status !== 'cancelled',
            )
            const memberCompletedTasks = memberTaskList.filter(
              (task) => task.status === 'completed',
            )
            const memberOverdueTasks = memberTaskList.filter(isOverdue)
            const completionRate = memberTaskList.length
              ? Math.round(
                  (memberCompletedTasks.length / memberTaskList.length) * 100,
                )
              : 0
            ...
            return (
              <Card
                key={member.id}
                className="p-5 transition-colors hover:border-[var(--os-border-strong)]"
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 text-xs font-medium text-[var(--os-accent)]">
                ...
                <div className="mt-5 space-y-3">
                  {member.email && (
                    <div className="flex min-w-0 items-center gap-2 text-sm text-[var(--os-text-secondary)]">
                      <Mail size={15} className="shrink-0 text-[var(--os-text-muted)]" />
                  ...
                  <div className="border-t border-[var(--os-border)] pt-3">
                    ...
                    <div className="mt-2 h-2 overflow-hidden rounded-full bg-[var(--os-surface-hover)]">
                      <div
                        className="h-full rounded-full bg-[var(--os-success)] transition-all"
                        style={{ width: `${completionRate}%` }}
                      />
                    </div>
                  </div>

                  <div className="flex items-center justify-between gap-3 border-t border-[var(--os-border)] pt-3">
                    <span className="text-xs text-[var(--os-text-muted)]">Role</span>
                    <span className="rounded-full bg-[var(--os-surface-hover)] px-2.5 py-1 text-xs font-medium capitalize text-[var(--os-text-secondary)]">
