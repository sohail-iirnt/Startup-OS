...
import { AlertTriangle, CalendarClock, CircleDollarSign, Edit3, FolderKanban, Plus, Search, Trash2, UserRound } from 'lucide-react'
...
type Sort = 'recent' | 'deadline' | 'value' | 'priority' | 'name'
...
function deadlineHealth(project: Project) { if (project.status === 'completed') return { label: 'Delivered', className: 'bg-[rgba(66,211,146,0.12)] text-[var(--os-success)]' }; if (project.status === 'cancelled') return { label: 'Cancelled', className: 'bg-[rgba(255,100,124,0.12)] text-[var(--os-danger)]' }; if (!project.deadline) return { label: 'No deadline', className: 'bg-[rgba(255,255,255,0.06)] text-[var(--os-text-muted)]' }; const days = Math.ceil((project.deadline.getTime() - Date.now()) / 86400000); if (days < 0) return { label: `${Math.abs(days)}d overdue`, className: 'bg-[rgba(255,100,124,0.12)] text-[var(--os-danger)]' }; if (days <= 7) return { label: days === 0 ? 'Due today' : `Due in ${days}d`, className: 'bg-[rgba(245,185,66,0.12)] text-[var(--os-warning)]' }; return { label: 'On track', className: 'bg-[rgba(66,211,146,0.12)] text-[var(--os-success)]' } }
...
function Projects() {
  ...
  const filtered = useMemo(() => {
    ...
    const matches = projects.filter((p) => (filter === 'all' || p.status === filter) && (!q || [p.name, p.clientName, p.type, p.priority, p.ownerName].some((v) => v.toLowerCase().includes(q))))
    return [...matches].sort((a, b) => {
      if (sort === 'name') return a.name.localeCompare(b.name)
      if (sort === 'value') return b.projectValue - a.projectValue
      if (sort === 'priority') { const rank: Record<ProjectPriority, number> = { urgent: 4, high: 3, medium: 2, low: 1 }; return rank[b.priority] - rank[a.priority] }
      if (sort === 'deadline') { if (!a.deadline && !b.deadline) return 0; if (!a.deadline) return 1; if (!b.deadline) return -1; return a.deadline.getTime() - b.deadline.getTime() }
      return b.createdAt.getTime() - a.createdAt.getTime()
    })
  }, [projects, search, filter, sort])
  const stats = useMemo(() => { const atRisk = projects.filter((p) => { const health = deadlineHealth(p); return health.label.includes('overdue') || health.label.includes('Due ') }).length; return { total: projects.length, active: projects.filter((p) => p.status === 'in-development').length, completed: projects.filter((p) => p.status === 'completed').length, atRisk, value: projects.reduce((sum, p) => sum + p.projectValue, 0) } }, [projects])
