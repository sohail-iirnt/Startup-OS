import { collection, doc, onSnapshot, query, serverTimestamp, where, writeBatch, type Unsubscribe } from 'firebase/firestore'
...
export async function savePayroll(workspaceId: string, userId: string, form: PayrollForm, existingId?: string): Promise<void> {
  ...
  const payrollData = {
    workspaceId,
    paidToUserId: form.paidToUserId || null,
    paidTo: form.paidTo.trim(),
    recipientType: form.recipientType,
    paymentType,
    customPaymentType: form.paymentType === 'custom' ? form.customPaymentType.trim() : null,
    paidDate: new Date(`${form.paidDate}T12:00:00`),
  ...
  batch.set(financeRef, {
    ...
    amount: totalAmount,
    category: 'Payroll',
    description: `${paymentType} payment to ${form.paidTo.trim()}`,
    date: new Date(`${form.paidDate}T12:00:00`),
    method: form.paymentMethod,
    party: form.paidTo.trim(),
    partyType: form.recipientType,
    payPeriod: form.periodStart && form.periodEnd ? `${form.periodStart} to ${form.periodEnd}` : '',
