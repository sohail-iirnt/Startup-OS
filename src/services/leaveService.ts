...
function mapRequest(id: string, data: Record<string, unknown>): LeaveRequest { return { id, workspaceId: String(data.workspaceId ?? ''), userId: String(data.userId ?? ''), type: String(data.type ?? 'other') as LeaveType, startDate: String(data.startDate ?? ''), endDate: String(data.endDate ?? ''), reason: String(data.reason ?? ''), status: String(data.status ?? 'pending') as LeaveStatus, reviewedBy: data.reviewedBy ? String(data.reviewedBy) : undefined, reviewNote: data.reviewNote ? String(data.reviewNote) : undefined, createdAt: toDate(data.createdAt), updatedAt: toDate(data.updatedAt) } }
...
export async function createLeaveRequest(input: { workspaceId: string; userId: string; type: LeaveType; startDate: string; endDate: string; reason: string }) { await addDoc(collection(db, COLLECTION), { ...input, status: 'pending', createdAt: serverTimestamp(), updatedAt: serverTimestamp() }) }
export async function reviewLeaveRequest(id: string, status: Extract<LeaveStatus, 'approved' | 'rejected'>, reviewedBy: string, reviewNote = '') { await updateDoc(doc(db, COLLECTION, id), { status, reviewedBy, reviewNote, updatedAt: serverTimestamp() }) }
