...
export async function requestWorkspaceMembership(workspaceId: string, user: User, requestedRole: UserRole = 'member'): Promise<WorkspaceMember> {
  ...
  if (existingSnapshot.exists()) {
    ...
    if (existing.status === 'active') { await setDefaultWorkspace(user.uid, normalizedWorkspaceId); return existing }
    if (existing.status === 'pending' || existing.status === 'invited') { await setDefaultWorkspace(user.uid, normalizedWorkspaceId); return existing }
